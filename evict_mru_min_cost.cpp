...
int main() {
    ...
    for (int time=1; time<=M; time++) {
        ...
        if (bufferSize[u] == qmax[u]) {
            ...
            for (auto it = timeToSlot[u].rbegin(); it != timeToSlot[u].rend(); it++) {
                --cnt;
                if (cnt == 0)
                    break;
                ...
                if (get<0>(user_page) != u) {
                    throw new std::invalid_argument("get out !!!");
                }
                int page = get<1>(user_page);

                double weight = 0;
                if (e_faultcount[u][page] == 0) {
                    if (faultcount[u][page] == 0) {
                        weight = 0;
                    } else {
                        weight = faultcount[u][page];
                    }
                } else {
                    weight = 1.0 * (faultcount[u][page] - e_faultcount[u][page] ) / e_faultcount[u][page];
                }
            ...
            int slot = timeToSlot[u][access_time];
