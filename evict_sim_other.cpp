...
int main() {
    ...
    int maxReRefDistance[N + 1][100100];
    for (int i=1; i<=N; i++) {
        bufferSize[i] = 0;
        tenantTime[i] = 0;
        pageFault[i] = 0;
    ...
    if (N <= 2 && maxL > 1.5 * minL + 1e-9) {
        md_optimize = true;
    }
    double md_factor = 0.05;
...
//if (get<1>(to_evict_hit_time) > tenantTime[evictable] - bufferSize[evictable] * md_factor) {
                    if (get<1>(hit_time) <= tenantTime[kd] - bufferSize[kd] * md_factor) {
...
//if (get<1>(to_evict_hit_time) > tenantTime[evictable] - bufferSize[evictable] * md_factor) {
        ...
        if (requestTimes[u][p].size() >= 2) {
            int szzz = requestTimes[u][p].size();
            maxReRefDistance[u][p] = max(maxReRefDistance[u][p],
                    requestTimes[u][p][szzz - 1] - requestTimes[u][p][szzz - 2]);
        }
        ...
        if (tenantTime[evictable] % 1000 == 0 && tenantTime[evictable] > deadline[evictable]) {
        ...
        if (N == 4) {
            ...
            if (hit_ratio < segment_ratio) {
                ...
                while (true) {
                    ...
                    if (get<1>(nextHitIt->first) < get<1>(to_evict_hit_time) ) {
                        to_evict_hit_time = nextHitIt->first;
                        break;
                    }
            ...
            while (true) {
                ...
                int cnt = 20;
                for (auto leveledIt = nextHitIt; leveledIt != hitsToSlot[evictable].end();
                            leveledIt++) {
                if (cnt == 0)
                    break;
                    cnt--;
                if (get<0>(leveledIt->first) != freq) {
                    break;
                }


                int oldTime = get<1>(leveledIt->first);
                //if ( (tenantTime[evictable] - oldTime) > 4 * (tenantTime[evictable] - get<1>(lfu_hit_time) )   ) {

                
                //if ( oldTime < get<1>(lfu_hit_time) && (tenantTime[evictable] - oldTime) > 6 * bufferSize[evictable] ) {
                if ( oldTime < get<1>(lfu_hit_time) ) {

                    auto quite_old_hit_time = leveledIt->first;
                    ...
                    tuple<int, int> youngified;
                    if (oldCount >= bufferSize[evictable] * 0.1) {
                        youngified = make_tuple(get<0>(quite_old_hit_time) - 1, get<1>(quite_old_hit_time));
                    } else {
                        youngified = make_tuple(get<0>(quite_old_hit_time), get<1>(quite_old_hit_time));
                    }
                    ...
                    for (int iii=1; iii < requestTimes[oldUser][oldPage].size(); iii++) {
                        averageReRefTime = max(averageReRefTime,  (double)(requestTimes[oldUser][oldPage][iii] -
                                                requestTimes[oldUser][oldPage][iii - 1]) );
                    }*/
                    double averageReRefTime = maxReRefDistance[oldUser][oldPage];
            ...
            if (leastProjectedTime < tenantTime[evictable] - 10 ) {
        ...
        if (md_optimize && get<1>(to_evict_hit_time) > tenantTime[evictable] - bufferSize[evictable] * md_factor) {
            md_tenant = evictable;
        }
